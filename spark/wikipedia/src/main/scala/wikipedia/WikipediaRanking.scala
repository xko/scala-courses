...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd.filter(_.mentionsLanguage(lang)).map(_=>1).fold(0)(_+_)
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    langs.map(l => (l, occurrencesOfLang(l, rdd))).filter(_._2 > 0).sortBy(- _._2)
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] =
    index.map {
      case (str, articles) => (str, articles.size)
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    rdd.flatMap( a => langs.filter(a.mentionsLanguage).map((_, 1)) ).reduceByKey(_+_)
      .sortBy(- _._2).collect.toList
