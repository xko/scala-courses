...
class HuffmanSuite {
  ...
  @Test def `times are correct`: Unit =
    assertEquals(List('c'->2,'b'->1,'a'->5), times(string2Chars("abaacaca")))
  ...
  @Test def `decode ab` : Unit = {
    new TestTrees {
      assertEquals(List('a','b'), decode(t1,List(0,1)))
    }
  }
