...
object ParallelCountChange extends ParallelCountChangeInterface:
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int =
    if threshold(money,coins) then countChange(money,coins)
      ...
      case Nil => 0
      case coin :: rest =>
        val (a,b) = parallel( parCountChange(money-coin,coins,threshold) , parCountChange(money,rest,threshold) )
        a + b
  ...
  def combinedThreshold(startingMoney: Int, allCoins: List[Int]): Threshold = {
    val n = (startingMoney * allCoins.size) / 2
    (money, coins) => money * coins.size <= n
  }
