...
class VisualizationTest extends AnyFunSpec with Matchers with ScalaCheckPropertyChecks with TestUtil {
  ...
  def visualizationImpl(impl: => VisualizationInterface): Unit ={
    import Visualization.{dSigma,Colors}
    ...
    describe("temperature interpolation") {
      ...
      describe("with 2 refs"){
        it("works on the edge") {
          impl.predictTemperature( List( Location(45.0, -90.0)->10.0, Location(-45.0, 0.0)->20.0 ),
          ...
          it("predicts temp closer to the closer ref") {
            ...
            forAll { (near:Location, tNear: Temperature, far:Location, tFar:Temperature, x: Location) =>
              whenever(dSigma(near, x) < dSigma(far, x)) {
                impl.predictTemperature( List(near->tNear, far->tFar), x ) should beCloserTo(tNear).thenTo(tFar)
              }
            }
          }
        }
    ...
    describe("color interpolation") {
      it("works on some points") {
        val color = impl.interpolateColor( List(1.0->Color(255, 0, 0), 2.0->Color(0, 0, 255), 3.0->Color(0, 255, 0)), 1.5 )
    ...
    describe("visualization"){
      it("writes image file") {
        val image = impl.visualize( List( Location(-170, 80)->33, Location(170, 80)->33,
      ...
      describe("with 2 refs"){
        ...
        it("works between") {
          val image = impl.visualize( List( Location(45.0, -90.0)->0.0, Location(-45.0, 0.0)->6.632951209392111 ),
        ...
        it("produces color closer to the closer ref") {
          ...
          forAll { (near:Location, tNear: Temperature, far:Location, tFar:Temperature, x: Location) =>
            whenever(dSigma(near, x) < dSigma(far, x)) {
              val image = impl.visualize( List(near->tNear, far->tFar),
  ...
  describe("plain implementation") {
    it should behave like visualizationImpl(Visualization.PlainImpl)
  }
